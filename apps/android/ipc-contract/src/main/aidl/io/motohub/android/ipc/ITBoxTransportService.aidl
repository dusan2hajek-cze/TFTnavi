...
interface ITBoxTransportService {
    ...
    /**
     * TFTnavi extension: starts Core's AVC encoder for a companion-rendered pixel source and
     * returns the encoder input Surface. Appended at the tail so every upstream 1.1.119
     * transaction id remains unchanged.
     */
    Surface startExternalVideoSurface();

    /** Stops the TFTnavi external Surface encoder. Safe when no such session is active. */
    void stopExternalVideoSurface();
}
